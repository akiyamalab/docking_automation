...
class TestTask:
    ...
    def sample_task(self, sample_function):
        ...
        return Task(id="task1", function=sample_function, args={"a": 1, "b": 2})

    @pytest.mark.skip(reason="未実装のテスト")
    def test_initialization(self, sample_function):
        """初期化のテスト"""
        task = Task(id="task1", function=sample_function, args={"a": 1, "b": 2})
    ...
    def test_execute_with_invalid_args(self, sample_function):
        """無効な引数でのタスク実行のテスト"""
        task = Task(id="task1", function=sample_function, args={"a": 1, "c": 2})  # 'b'が欠けている
