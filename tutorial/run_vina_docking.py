#!/usr/bin/env python3
"""
実際のVinaコマンドを使用してドッキング計算を実行するスクリプト
"""
...
# VinaサービスとMockMoleculePreparationServiceをインポート
...
def main():
    ...
    # 入力ファイルの確認
    ligand_path = os.path.join(current_dir, "protein.pdb")
    ...
    ligand_format = MoleculeFormat(type=FormatType.from_path(ligand_path))
    compound = Compound(
        id="test_ligand",
    ...
    prepared_compound = molecule_preparation_service.prepare_ligand(compound)
    
    # 2. タンパク質のロードと準備
    logger.info(f"Loading receptor: {receptor_path}")
    receptor_format = MoleculeFormat(type=FormatType.from_path(receptor_path))
    protein = Protein(
        id="test_receptor",
        structure=MoleculeStructure(atoms=[], bonds=[]),  # ダミー構造（実際にはファイルから読み込む）
    ...
    prepared_protein = molecule_preparation_service.prepare_receptor(protein)
            ...
            try:
                if hasattr(result.task, 'get_primary_receptor') and callable(getattr(result.task, 'get_primary_receptor')):
                    ...
                    if isinstance(receptor, list):
                        ...
                        receptor_name = receptor.name if hasattr(receptor, 'name') else "Unknown"
