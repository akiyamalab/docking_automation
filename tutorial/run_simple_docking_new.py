#!/usr/bin/env python3
"""
単純な分子を使ったドッキング計算のデモンストレーション（新しいインポート構造を使用）
...
def create_simple_molecule(smiles: str, mol_id: str):
    ...
    for bond in mol.GetBonds():
        bonds.append(Bond(
            ...
            bond_type="single" if bond.GetBondType() == Chem.BondType.SINGLE else 
                     "double" if bond.GetBondType() == Chem.BondType.DOUBLE else
                     "triple" if bond.GetBondType() == Chem.BondType.TRIPLE else
                     "aromatic"
        ))
...
def prepare_ligand_with_meeko(compound):
    """Meekoを使ってリガンドを準備"""
    ...
    try:
        # RDKitで分子を読み込み
        mol = Chem.MolFromPDBFile(compound.path, removeHs=False)
        if mol is None:
        ...
        # docking_automation.moleculeのCompoundクラスの属性を設定
...
def main():
    # 出力ディレクトリの作成
    output_dir = os.path.join(CURRENT_DIR, "simple_docking_results_new")
    ...
    protein = Protein(
        id="receptor",
        structure=MoleculeStructure(atoms=[], bonds=[]),  # 簡略化のため空の構造
        format=MoleculeFormat(type=FormatType.PDBQT),
        path=receptor_path,
        is_prepared=True  # すでにPDBQT形式なので準備済み
    )
    ...
    for molecule_data in molecules:
        ...
        # リガンド準備
        prepared_compound = prepare_ligand_with_meeko(molecule_data)
        if not prepared_compound:
        ...
        docking_result = run_vina_docking(
            ligand_path=prepared_compound.path,
        ...
        results.append({
            "id": molecule_data.id,
            "properties": molecule_data.metadata,
