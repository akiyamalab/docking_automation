...
def parse_args():
    ...
    parser.add_argument(
        "--energy-range", type=float, default=3.0, help="出力するポーズのエネルギー範囲（デフォルト: 3.0）"
    )
    ...
    try:
        ...
        if max_compounds is not None and max_compounds > 0:
            if max_compounds < total_compounds:
                ...
                print(
                    f"指定された最大数（{max_compounds}）が全化合物数（{total_compounds}）以上のため、すべての化合物を処理します"
                )
                max_compounds = None
        ...
        print(
            f"ドッキングパラメータ: exhaustiveness={args.exhaustiveness}, num_modes={args.num_modes}, energy_range={args.energy_range}"
        )
