...
class MockDockingService(DockingService):
    ...
    def execute(self, task: DockingTask) -> DockingResult:
        ...
        # ランダムなスコアを生成
        scores = [Score(value=random.uniform(-12.0, -5.0), type="vina", rank=i+1) for i in range(3)]
        
        # モックのポーズを生成
        poses = [Pose(content=f"ATOM  12345  CA  ALA A  42      {task.configuration.grid_box.center_x:.3f} {task.configuration.grid_box.center_y:.3f} {task.configuration.grid_box.center_z:.3f}", rank=i+1) for i in range(3)]
        
        # 結果を返す
        return DockingResult(
            task_id=task.id,
            ligand_id=task.ligand.get_id(),
            scores=scores,
            poses=poses,
            success=True,
            error_message=None
...
for i, mol in enumerate(mols):
    ...
    with open(result_path, "w") as f:
        f.write(result.poses[0].content)
