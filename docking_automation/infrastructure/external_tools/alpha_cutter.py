...
for pdb in filepath_list:
    ...
    for chain_id, chain_df in pdb_df_dict.items():
        ...
        # groupby residue
        pdb_df_dict_groupby[chain_id] = chain_df.groupby("residue_number").agg(lambda x: safe_mode(x))
        print(pdb_df_dict_groupby[chain_id].iloc[0])
        print(pdb_df_dict_groupby[chain_id].describe())
    ...
    if loop_min_len == None:
        ...
        loop_list_dict = {
            chain_id: get_helix_loop_list("-", chain_df, contact_list_dict[chain_id], loop_min_len)
            for chain_id, chain_df in pdb_df_dict_groupby.items()
        }
        loop_list_merged_dict = {
            chain_id: merge_list(
                get_helix_loop_list("-", chain_df, contact_list_dict[chain_id], loop_min_len), chain_df
            )
            for chain_id, chain_df in pdb_df_dict_groupby.items()
        }
    ...
    if helix_min_len == None:
        ...
        helix_list_dict = {
            chain_id: get_helix_loop_list("H", chain_df, contact_list_dict[chain_id], helix_min_len)
            for chain_id, chain_df in pdb_df_dict_groupby.items()
        }
        helix_list_merged_dict = {
            chain_id: merge_list(
                get_helix_loop_list("H", chain_df, contact_list_dict[chain_id], helix_min_len), chain_df
            )
            for chain_id, chain_df in pdb_df_dict_groupby.items()
        }
        ...
        if len(domain_df_list) > 0:
            ...
            if all([domain_pLDDT_mean >= pLDDT_min, domain_len >= domain_min_len]):
                ...
                for chain_id in domain_df_chain_res_unique["chain_id"].unique():
                    start = domain_df_chain_res_unique.loc[domain_df_chain_res_unique["chain_id"] == chain_id][
                        "residue_number"
                    ].min()
                    end = domain_df_chain_res_unique.loc[domain_df_chain_res_unique["chain_id"] == chain_id][
                        "residue_number"
                    ].max()
                ...
                if domain_out == True:
                    ...
                    ppdb.to_pdb(
                        path=pdb.replace(".pdb", "") + "_AFCT-OUT_domain_" + output_range + ".pdb",
                        records=None,
                        gz=False,
                        append_newline=True,
                    )
    ...
    if all([single_out == True, len(domain_df_list_single) > 0]):
        ...
        for chain_id in domain_df_single_chain_res_unique["chain_id"].unique():
            start = domain_df_single_chain_res_unique.loc[domain_df_single_chain_res_unique["chain_id"] == chain_id][
                "residue_number"
            ].min()
            end = domain_df_single_chain_res_unique.loc[domain_df_single_chain_res_unique["chain_id"] == chain_id][
                "residue_number"
            ].max()
            output_range_list.append(chain_id + str(start) + "-" + str(end))
        ...
        if len(domain_list) > 0:
            ...
            for domain in domain_list:
                domain_df = pdb_df.loc[
                    (pdb_df["residue_number"] >= domain[0]) & (pdb_df["residue_number"] <= domain[1])
                ]
...
for pdb in summary_df_all["PDB"].unique():
    ...
    for chain_id in summary_df_all_pdb["chainID"].unique():
        ...
        for idx, row in summary_df_all_pdb_one.iterrows():
            ...
            start_idx = pdb_df.loc[(pdb_df["chain_id"] == chain_id) & (pdb_df["residue_number"] == row["start"])].index[
                0
            ]
