...
class AutoDockVina(DockingToolABC):
    ...
    def dock(self, parameters: DockingParameters) -> List[DockingResult]:
        ...
        # 化合物の数を取得
        total_compounds = len(compound_set.file_paths)
        
        # 処理する化合物の数を決定
        if max_compounds is not None and max_compounds > 0 and max_compounds < total_compounds:
            compounds_to_process = compound_set.file_paths[:max_compounds]
            print(f"ドッキング計算を開始します（全{total_compounds}化合物中、最初の{max_compounds}化合物）...")
        else:
            compounds_to_process = compound_set.file_paths
            print(f"ドッキング計算を開始します（全{total_compounds}化合物）...")
