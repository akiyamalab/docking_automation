...
# N_CMPDS_PER_FILE=$(python count.py Compound_119500001_120000000.sdf 500)
N_CMPDS_PER_FILE=$1

LAST=$2
echo $SLURM_ARRAY_TASK_ID
echo $N_CMPDS_PER_FILE
echo $LAST

start=$(date +%s)
# TIME1=$(cat /proc/uptime | awk '{print $1}')
# echo time1: $TIME1
#python auto_docking.py "PubChem_c_${SLURM_ARRAY_TASK_ID}.sdf" protein.pdb $N_CMPDS_PER_FILE "${SLURM_ARRAY_TASK_ID}"
python auto_docking.py "${LAST}" protein.pdb "${N_CMPDS_PER_FILE}" "${SLURM_ARRAY_TASK_ID}"
