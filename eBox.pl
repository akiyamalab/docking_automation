...
for ($i=0; $i< $atoms; $i++){
    if ($max_dis < ($locs[$i*3+0]-$geo_center[0])**2 + ($locs[$i*3+1]-$geo_center[1])**2 + ($locs[$i*3+2]-$geo_center[2])**2) {
	$max_dis = ($locs[$i*3+0]-$geo_center[0])**2 + ($locs[$i*3+1]-$geo_center[1])**2 + ($locs[$i*3+2]-$geo_center[2])**2;
    }
...
#printf("# of all   atoms: %d\n", $ct);
#printf("# of heavy atoms: %d\n", $atoms);
printf("%d\n", int($gyration_g/2+0.9999)*2);
printf("%.4f %.4f %.4f\n", $geo_center[0], $geo_center[1], $geo_center[2]);

#printf("ligand's max radius: %f\n", sqrt($max_dis));
